...
const tzLocal = {
	...
	co2_config: {
        key: ['auto_brightness', 'onoff_brightness', 'forced_recalibration', 'factory_reset_co2', 'long_chart_period', 'manual_forced_recalibration', 'alarm', 'rotate', 'automatic_scal'],
        convertSet: async (entity, key, rawValue, meta) => {
			const endpoint = meta.device.getEndpoint(1);
            const lookup = {'OFF': 0x00, 'ON': 0x01};
            const value = lookup.hasOwnProperty(rawValue) ? lookup[rawValue] : parseInt(rawValue, 10);
            const payloads = {
				...
				alarm: ['msCO2', {0x0240: {value, type: 0x10}}],
...
const fzLocal = {
	...
	co2_config: {
        ...
        convert: (model, msg, publish, options, meta) => {
			...
			if (msg.data.hasOwnProperty(0x0240)) {
                result.alarm = ['OFF', 'ON'][msg.data[0x0240]];
            }
...
const definition = {
        ...
        exposes: [e.co2(), e.temperature(), e.humidity(), e.illuminance(), e.pressure(),
			...
			exposes.binary('alarm', ea.STATE_SET, 'ON', 'OFF')
			    .withDescription('Audible alarm when the CO2 level is exceeded'),
